...
const App: React.FC = () => {
  ...
  const [data, setData] = useState<FinancialData>(() => {
    ...
    return {
      cardTransactions: [],
      fixedExpenses: [
        { id: '1', name: 'Água', amount: 0, dueDate: '10', isPaid: false, category: FixedExpenseCategory.AGUA, month: now.getMonth(), year: now.getFullYear() },
        { id: '2', name: 'Luz', amount: 0, dueDate: '15', isPaid: false, category: FixedExpenseCategory.LUZ, month: now.getMonth(), year: now.getFullYear() },
        { id: '3', name: 'Internet Casa', amount: 0, dueDate: '05', isPaid: false, category: FixedExpenseCategory.INTERNET_CASA, month: now.getMonth(), year: now.getFullYear() },
        { id: '4', name: 'Terreno', amount: 0, dueDate: '20', isPaid: false, category: FixedExpenseCategory.TERRENO, month: now.getMonth(), year: now.getFullYear() },
        { id: '5', name: 'TV', amount: 0, dueDate: '01', isPaid: false, category: FixedExpenseCategory.TV, month: now.getMonth(), year: now.getFullYear() },
      ],
      incomes: []
    };
  });
  ...
  const handleAddCardTransaction = (base: Omit<CardTransaction, 'id'>, targetMonths: number[]) => {
    const year = currentDate.getFullYear();
    const newTransactions: CardTransaction[] = targetMonths.map(m => ({
      ...base, id: `${Date.now()}-${m}-${Math.random()}`,
      purchaseDate: new Date(year, m, 1).toISOString()
    }));
    setData(prev => ({ ...prev, cardTransactions: [...prev.cardTransactions, ...newTransactions] }));
    setShowModal(null);
  };

  const handleAddIncome = (base: Omit<Income, 'id'>, targetMonths: number[]) => {
    const year = currentDate.getFullYear();
    const newIncomes: Income[] = targetMonths.map(m => ({
      ...base, id: `${Date.now()}-${m}-${Math.random()}`,
      date: new Date(year, m, 1).toISOString()
    }));
    setData(prev => ({ ...prev, incomes: [...prev.incomes, ...newIncomes] }));
    setShowModal(null);
  };

  const handleAddFixedExpense = (base: Omit<FixedExpense, 'id' | 'isPaid' | 'month' | 'year'>, targetMonths: number[]) => {
    const year = currentDate.getFullYear();
    const newExpenses: FixedExpense[] = targetMonths.map(m => ({
      ...base, id: `${Date.now()}-${m}-${Math.random()}`,
      isPaid: false, month: m, year: year
    }));
    setData(prev => ({ ...prev, fixedExpenses: [...prev.fixedExpenses, ...newExpenses] }));
    setShowModal(null);
  };
  ...
  const updateFixedAmount = (id: string, amount: number) => {
    setData(prev => ({ ...prev, fixedExpenses: prev.fixedExpenses.map(e => e.id === id ? { ...e, amount } : e) }));
  };

  const deleteTransaction = (type: 'card' | 'income' | 'fixed', id: string) => {
